        ...
        public void Update(GameTime gameTime)
        {
            ...
            for (int i = 0; i < Players.Count; i++) {
                ...
                for (int j = 0; j < Players.Count; j++) {
                    ...
                    if (Collider.Collides(player, otherPlayer, true, false, false, false)){
                        if (player.Velocity.X > 0) {
                            player.Velocity =
                                new Vector2(
                                    (otherPlayer.Position.X - player.Position.X - 2*GameVariables.PlayerCollisionRadius) +
                                    GameVariables.PlayerPushSpeed*dt, player.Velocity.Y);
                            otherPlayer.Velocity = new Vector2(otherPlayer.Velocity.X + GameVariables.PlayerPushSpeed * dt,
                    ...
                    if (Collider.Collides(player, otherPlayer, false, true, false, false)){
                        if (player.Velocity.Y > 0) {
                            ...
                            player.Velocity = new Vector2(player.Velocity.X,
                                                          ...
                                                          GameVariables.PlayerPushSpeed * dt);
                            otherPlayer.Velocity = new Vector2(otherPlayer.Velocity.X,
                                                               otherPlayer.Velocity.Y + GameVariables.PlayerPushSpeed * dt);
                        }
